...
def fix_file(filepath):
    ...
    # Skip files that should not raise on failure (like long_term_memory or scripts)
    ...
    # Replace crypto_manager.decrypt(pref.value) with crypto_manager.decrypt(pref.value, raise_on_failure=True)
    # Be careful not to replace if raise_on_failure is already there
    new_content = re.sub(r'crypto_manager\.decrypt\(([^,)]+)\)', r'crypto_manager.decrypt(\1, raise_on_failure=True)', content)
    
    # Handle crypto_mod.crypto_manager.decrypt(...)
